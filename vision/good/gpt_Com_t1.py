...
class BallDetectorNode(Node):
    ...
    def __init__(self):
        ...
        # ── 网格密度聚类参数 ─────────────────────────────────
        self.grid_size          = 8    # 格子边长；原16在>1.5m时球体仅覆盖1~2格导致±16px量化，降至8减少一半
        self.show_heatmap       = True # 是否叠加密度热力图（调试用，上线可关）
        self.density_min_pixels = 5    # 峰值格子最低有效像素数；grid_size减半后格子缩小4x，相应降低
    ...
    def image_callback(self, color_msg, depth_msg):
        ...
        if self.prior_state == 'collecting':
            ...
            # ── 密度检测失败时回退到轮廓质心 ──
            if not found:
                mask_full = cv2.bitwise_or(mask_yellow, mask_blue)
                # 适当地 dilate 连接黄蓝碎片，否则远距离两色不连通找不到有效轮廓
                kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
                mask_dilated = cv2.dilate(mask_full, kernel, iterations=2)
